...
class Game {
  ...
  initUI() {
    ...
    if (
      typeof localStorage.sound == "undefined" ||
      localStorage.sound == "true"
    ) {
      this.ui.sound.checked = true;
      this.sound = true;
    } else {
      this.ui.sound.checked = false;
      this.sound = false;
    }
    ...
    if (
      typeof localStorage.tips == "undefined" ||
      localStorage.tips == "false"
    ) {
      this.ui.tips.checked = false;
      this.tips = false;
    } else {
      this.ui.tips.checked = true;
      this.tips = true;
    }
  ...
  updateUI() {
    ...
    this.ui.accuracy.innerText =
      ((this.round.accuracy / this.round.word_count) * 100).toFixed(1) + "%";

    var wpm = (
      (Math.round(this.round.letter_count / 5, 0) / this.round.elapsed) *
      60
    ).toFixed(2);
  ...
  nextWord() {
    ...
    this.words.index =
      this.words.index + 1 >= this.words.length ? 0 : this.words.index + 1;
  ...
  score(word) {
    // successful pop of a word
    var score =
      word.length * (this.tips ? this.SCORE_LETTER / 2 : this.SCORE_LETTER);
